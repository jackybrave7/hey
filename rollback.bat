...
for /f "delims=" %%i in ('ssh -i "%SSH_KEY%" -o StrictHostKeyChecking=no -o ConnectTimeout=15 -o BatchMode=yes %SERVER% "test -f %APP_DIR%/server/data/.last_good_commit && cat %APP_DIR%/server/data/.last_good_commit || (test -f %APP_DIR%/.last_good_commit && cat %APP_DIR%/.last_good_commit) || echo __MISSING__"') do set "TARGET=%%i"
...
ssh -i "%SSH_KEY%" -o StrictHostKeyChecking=no -o ConnectTimeout=15 -o BatchMode=yes %SERVER% "cd %APP_DIR% && echo HEAD: && git rev-parse --short HEAD && git log -1 --oneline"
...
ssh -i "%SSH_KEY%" -o StrictHostKeyChecking=no -o ConnectTimeout=15 -o BatchMode=yes %SERVER% "docker start hey 2>/dev/null; cd %APP_DIR% && git fetch -q origin && git checkout -f feat/messaging-extras 2>/dev/null || git checkout -B feat/messaging-extras && git reset --hard %TARGET%"
...
ssh -i "%SSH_KEY%" -o StrictHostKeyChecking=no -o ConnectTimeout=15 -o BatchMode=yes %SERVER% "docker restart hey && sleep 2 && curl -sf %HEALTH_URL%"
